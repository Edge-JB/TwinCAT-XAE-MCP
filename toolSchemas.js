...
const toolSchemas = {
  ...
  plc_pou: {
    description:
      ...
      "FOLDERS — create_folder (parent, name, before?) creates a PLC folder (sub-type 601) under parent (a PLC subtree node, POUs/DUTs/GVLs container, or another folder); returns the same shape as create. create_folder_batch (creates:[{parent,name,before?}], save?) loops continue-on-error, returns {count,succeeded,failed,results} (FAILURES-ONLY; details:true for every row) — list a parent-folder entry before its child (array order). create / create_batch already author INTO a folder when parent is the folder's path — no separate action needed. " +
    ...
    inputSchema: {
      ...
      save: z.boolean().optional(),
      details: z.boolean().optional().describe("*_batch: include ok:true rows (create_batch ok-rows carry child identity); default failures-only ({count,succeeded,failed} always reported)"),
