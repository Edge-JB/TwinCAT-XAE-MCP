...
contract between the Node front and the native daemon (and the legacy PowerShell bridge), and the
rules every change must respect. For the full design, see [docs/architecture.md](docs/architecture.md).
...
MCP client ──stdio──► index.js (Node 20) ──named pipe──► Te1000Daemon.exe ──COM/DTE──► XAE Shell
                       ...
                       daemonClient.js
                          │
                          └─ fallback ──spawns──► PowerShell ─► powershell/te1000-bridge.ps1 ─► XAE
```
...
- **`index.js`** — registers MCP tools, validates input with [zod](https://zod.dev), enforces
  ...
  Routes those actions to the daemon by default; falls back to the legacy bridge when
  `TE1000_NO_DAEMON=1`, no x64 shell is present, or the daemon is unreachable. In legacy mode it
  also owns the modal-dialog watchdog and pre-flight gate.
...
- **`daemonClient.js`** — named-pipe client: auto-spawns the daemon (detached, single-instance),
  ...
  the same user-facing error text the legacy watchdog produced.
...
- **`powershell/te1000-bridge.ps1`** — the legacy bridge, retained unchanged as a full fallback.
  It attaches to the running XAE Shell via its COM ProgID and calls the TE1000 Automation Interface,
  re-checking guards defensively.
- **`powershell/dialog-watch.ps1`**, **`dialog-allowlist.json`**, **`plc-session.ps1`** — the
  legacy reliability layer (the daemon reads the same allowlist; see
...
- An action handler is a 1:1 port of its PS bridge case via late-bound `dynamic` COM, returning the
  identical `data` JSON shape; the dispatcher wraps it as `{ok:true, result:<data>}`. Throw
  `BridgeException` for handled errors.
- For pure-Node work (front, schemas, client) you don't need to rebuild the daemon. Set
  `TE1000_NO_DAEMON=1` to run entirely through the legacy bridge while iterating.
...
`node daemon/test-ping.js` validates the daemon end to end **without** XAE. The PowerShell engine
tests under `powershell/test-code-engine*.ps1` are run by hand on a machine with XAE present.
...
2. Define the input schema in **`toolSchemas.js`** (the single source of truth) — not inline in
   `index.js`.
3. Port the handler in **both** worlds so behavior is identical with or without the daemon: a C#
   handler in `daemon/Actions/<group>Actions.cs` (registered in `Dispatcher.cs`) **and** the
   matching PS bridge case. Both must return the same `data` JSON.
...
- **Guard every live-target, destructive, or licensing action** with a `confirm` token, enforced
  in `index.js` (and re-checked defensively in the bridge). Off by default.
- **Never write toward the safety project.** Authoring tools must refuse safety-rooted (`TISC`)
  paths — `Assert-NotSafetyPath` in the bridge, `PathUtil.AssertNotSafetyPath` in the daemon. The
  guard is **case-insensitive** (regex `^\s*TISC(\^|$)`, IgnoreCase). Nothing in this toolchain may
  write toward the TwinSAFE/TISC safety system; safety stays read-only/diagnostic.
- **Never auto-answer destructive dialogs.** The dialog allowlist (read by both the daemon and the
  legacy watcher) must not contain Activate Configuration, Run-mode, restart, download, or safety
  prompts.
